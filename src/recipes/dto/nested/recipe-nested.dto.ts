...
export class IngredientDto {
  ...
  @ApiProperty({ 
    ...
    enum: ['gramos', 'cucharadas', 'kilogramo', 'mililitros', 'tazas', 'unidad', 'pizca']
  })
  measureType: string;
}
...
export class StepDto {
  ...
  @ApiProperty({ example: 'Preparación de la masa', description: 'Título del paso' })
  title: string;

  @ApiProperty({ example: 'Mezclar los ingredientes secos', description: 'Descripción detallada del paso' })
  description: string;
...
export class MediaResourceDto {
  @ApiProperty({ example: 'https://example.com/main.jpg', description: 'URL del recurso multimedia' })
  url: string;
...
export class RatingDto {
  @ApiProperty({ example: 'user123', description: 'ID del usuario que califica' })
  userId: string;
